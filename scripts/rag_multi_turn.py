...
def init_agent():
    ...
    # 1. Tool cho tìm kiếm web (Tavily)
    # Khởi tạo tool tìm kiếm
    search_tool = DuckDuckGoSearchRun()

    # 2. Tool cho truy vấn SQL nội bộ
    ...
    8.  **Sử dụng các phép JOIN khi cần thiết** để kết hợp dữ liệu từ nhiều bảng. Nhất là khi cảm giác không có thông tin đầy đủ.
    9.  **CẦN XÁC ĐỊNH CÁC TỪ ĐỒNG NGHĨA, VIẾT TẮT SAO CHO PHÙ HỢP VỚI DỮ LIỆU. TỪ ĐÓ TRUY VẤN TÊN CỘT (THUỘC TÍNH) HOẶC TÊN BẢN GHI CHÍNH XÁC.** Ví dụ: "HN" = "Hà Nội", v.v...

    **ĐẶC BIỆT, ĐỐI VỚI NHỮNG CÂU HỎI KHÓ, PHỨC TẠP, THÌ CẦN BÓC TÁCH RA. TRƯỚC HẾT HÃY LƯỚT QUA THÔNG TIN CỦA CÁC BẢNG VÀ XÁC ĐỊNH BẢNG NÀO CÓ THỂ CUNG CẤP THÔNG TIN. RỒI MỚI TẠO TRUY VẤN SQL.**
    **KHÔNG TỪ BỎ SỚM.**
    ...
    # Custom SQLDatabaseChain với kiểm tra SQL kỹ hơn
    import sqlparse
        ...
        def _call(
            ...
            try:
                ...
                if not self.use_query_checker:
                    ...
                    if not is_valid_sql(checked_sql_command):
                        return {self.output_key: f"Không thể thực hiện truy vấn vì câu lệnh SQL không hợp lệ: {checked_sql_command}"}
    ...
    # 3. Agent gồm LLM và tool. Tool ở đây là sql_chain và search web. (Tức là có 2 con LLMs + 1 con search web + 1 con SQL)
    # Tool cho Agent
    tools = [
        Tool(
            name="SQL Database",
        ...
        Tool(
            ...
            description=(
                "Dùng để tìm kiếm thông tin trên internet khi dữ liệu không có trong cơ sở dữ liệu."
            )
        )
    ]

    #Memory cho multi-turn
